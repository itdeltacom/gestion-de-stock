@extends('layouts.app')
@section('title', 'Modifier le Transfert #' . $stockTransfer->id)
            ...
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="d-flex align-items-center">
                        <h6>Modifier le Transfert #{{ $stockTransfer->id }}</h6>
                        <div class="ms-auto">
                            <a href="{{ route('stock-transfers.show', $stockTransfer->id) }}" class="btn bg-gradient-secondary btn-sm mb-0">
                        ...
                        <div class="row">
                                ...
                                <select class="form-select" id="from_warehouse_id" name="from_warehouse_id" required>
                                    ...
                                    @foreach($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}" 
                                                {{ $stockTransfer->from_warehouse_id == $warehouse->id ? 'selected' : '' }}>
                                            {{ $warehouse->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="to_warehouse_id" name="to_warehouse_id" required>
                                    ...
                                    @foreach($warehouses as $warehouse)
                                        <option value="{{ $warehouse->id }}" 
                                                {{ $stockTransfer->to_warehouse_id == $warehouse->id ? 'selected' : '' }}>
                                            {{ $warehouse->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="transfer_date" name="transfer_date" 
                                       value="{{ $stockTransfer->transfer_date->format('Y-m-d') }}" required>
                            </div>

                            <div class="mb-3">
                                <label for="note" class="form-label">Note</label>
                                <textarea class="form-control" id="note" name="note" rows="3" 
                                          placeholder="Note optionnelle sur le transfert">{{ $stockTransfer->note }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="current-products">
                                    @if($stockTransfer->details->count() > 0)
                                        @foreach($stockTransfer->details as $detail)
                                            <div class="product-item" data-product-id="{{ $detail->product_id }}">
                                                <div class="row align-items-center">
                                                    ...
                                                    <div class="col-md-3">
                                                        ...
                                                        <div class="stock-info">
                                                            Stock disponible: <span class="stock-warning">{{ $detail->product->stocks->where('warehouse_id', $stockTransfer->from_warehouse_id)->first()->quantity ?? 0 }}</span>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3">
                                                        <div class="text-muted">
                                                            <small>Référence: {{ $detail->product->reference ?? 'N/A' }}</small>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-2 text-end">
                                                        <i class="fas fa-trash remove-product" data-product-id="{{ $detail->product_id }}"></i>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                ...
                                <div class="card">
                                    <div class="card-body text-center">
                                        <button type="submit" class="btn bg-gradient-success btn-lg me-3">
                                            <i class="fas fa-save"></i> Modifier le Transfert
                                        </button>
                                        <a href="{{ route('stock-transfers.show', $stockTransfer->id) }}" class="btn bg-gradient-secondary btn-lg">
...
@push('js')
    ...
    <script>
        ...
        let currentProducts = @json($stockTransfer->details);
        ...
        $(document).ready(function() {
            ...
            // Initialize current products
            currentProducts.forEach(detail => {
                selectedProducts.push({
                    id: detail.product_id,
                    name: detail.product.name,
                    code: detail.product.code,
                    reference: detail.product.reference,
                    stock: detail.product.stocks.find(s => s.warehouse_id == {{ $stockTransfer->from_warehouse_id }})?.quantity || 0,
        ...
        function updateProductsDisplay() {
            const container = $('#productsContainer .current-products');
                    ...
                    html += `
                        <div class="product-item" data-product-id="${product.id}">
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-2 text-end">
                                    <i class="fas fa-trash remove-product" data-product-id="${product.id}"></i>
                                </div>
                            </div>
                        </div>
                    `;
        ...
        function submitForm() {
            ...
            $.ajax({
                url: "{{ route('stock-transfers.update', $stockTransfer->id) }}",
                type: 'POST',
                data: formData,
