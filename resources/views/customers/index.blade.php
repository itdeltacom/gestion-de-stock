...
@section('content')
                        ...
                        <div class="ms-auto">
                            @can('customer-create')
                                <button type="button" class="btn bg-gradient-primary btn-sm mb-0" data-bs-toggle="modal"
                                    data-bs-target="#customerModal" id="createBtn">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="customerModalLabel">Nouveau Client</h5>
                ...
                <form id="customerForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="credit_limit" class="form-label">Limite de Crédit (DH) <span
                                        class="text-danger">*</span></label>
                                <input type="number" step="0.01" class="form-control" id="credit_limit" name="credit_limit"
                                    value="0" required>
                                <div class="invalid-feedback" id="credit_limit_error"></div>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="increaseCreditModal" tabindex="-1" aria-labelledby="increaseCreditModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="increaseCreditModalLabel">Augmenter la Limite de Crédit</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="increaseCreditForm">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="current_credit_limit" class="form-label">Limite Actuelle</label>
                            <input type="text" class="form-control" id="current_credit_limit" readonly>
                        </div>

                        <div class="mb-3">
                            <label for="new_credit_limit" class="form-label">Nouvelle Limite <span
                                    class="text-danger">*</span></label>
                            <input type="number" step="0.01" class="form-control" id="new_credit_limit"
                                name="new_credit_limit" required>
                            <div class="invalid-feedback" id="new_credit_limit_error"></div>
                        </div>
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                        <button type="submit" class="btn btn-warning" id="creditSubmitBtn">
                            <span id="creditSubmitBtnText">Augmenter</span>
                            <span id="creditSubmitBtnSpinner" class="spinner-border spinner-border-sm d-none ms-2"
        ...
        $(document).ready(function () {
            ...
            let table = $('#customersTable').DataTable({
                ...
                columns: [
                    ...
                    { data: 'credit_info', name: 'credit_limit', className: 'text-center', orderable: false },
                    { data: 'credit_usage', name: 'current_credit', className: 'text-center', orderable: false },
            ...
            $('#createBtn').click(function () {
                ...
                $('#customerModalLabel').text('Nouveau Client');
            ...
            // Edit Button Click
            $('#customersTable').on('click', '.edit-btn', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('#customerModalLabel').text('Modifier le Client');
            ...
            $('#customerForm').submit(function (e) {
                ...
                if (method === 'PUT') {
                    url = "{{ route('customers.index') }}/" + customerId;
                }
            ...
            // Increase Credit Button Click
            $('#customersTable').on('click', '.increase-credit-btn', function () {
                const id = $(this).data('id');
                const currentLimit = $(this).data('current');

                $('#credit_customer_id').val(id);
                $('#current_credit_limit').val(currentLimit + ' DH');
                $('#new_credit_limit').val(currentLimit);
            ...
            // Increase Credit Form Submit
            $('#increaseCreditForm').submit(function (e) {
                e.preventDefault();

                const customerId = $('#credit_customer_id').val();

                $('#creditSubmitBtn').prop('disabled', true);
                $('#creditSubmitBtnSpinner').removeClass('d-none');
                $('.form-control').removeClass('is-invalid');
                $('.invalid-feedback').text('');

                $.ajax({
                    url: "{{ route('customers.index') }}/" + customerId + "/increase-credit",
                    type: 'POST',
                    data: {
                        _token: $('input[name="_token"]').val(),
                    ...
                    success: function (response) {
                        if (response.success) {
                            const modal = bootstrap.Modal.getInstance(document.getElementById('increaseCreditModal'));
                            modal.hide();
                            table.ajax.reload();
                    ...
                    complete: function () {
                        $('#creditSubmitBtn').prop('disabled', false);
                        $('#creditSubmitBtnSpinner').addClass('d-none');
                    }
                });
            });

            // View Button Click
            $('#customersTable').on('click', '.view-btn', function (e) {
                e.preventDefault();
